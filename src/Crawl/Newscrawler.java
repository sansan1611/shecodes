...
public class Newscrawler {
	...
	private static final Map<String, NewsFormat[]> FORMATS = new HashMap<String, NewsFormat[]>() {
		private static final long serialVersionUID = -8049175202147902011L;
		{
			...
			put("vnexpress", new NewsFormat[] { new NewsFormat("article", "h3 > a", "div > a > picture > img") });
		}
	};
	...
	private List<Article> getArticlesFromProviderFormats(NewsFormat[] newsFormats, Document doc, String provider,
			String siteRoot) {
		List<Article> articles = new ArrayList<>();
	...
	private Article createArticleFrom(Element e, NewsFormat nf, String provider, String siteRoot) {
		try {
			...
			String absUrl = siteRoot
					.concat(e.selectFirst(headerSelector).attributes().get("href").replace(siteRoot, ""));
			...
			return new Article(provider, absUrl, heading, imgUrl);
		} catch (Exception ex) {
			return null;
		}
	}
